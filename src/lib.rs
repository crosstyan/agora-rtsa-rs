...
pub mod agoraRTC{
    ...
    impl From<RtcServiceOption> for rtc_service_option_t {
        fn from(opt: RtcServiceOption) -> Self {
            rtc_service_option_t { 
                ...
                product_id: opt.product_id.as_bytes().try_into().expect("Product ID is too long!"),
                log_cfg: opt.log_cfg.into(), 
                license_value: opt.license_value.as_bytes().try_into().expect("License Value is too long!"),
            }
        }
    }
    ...
    pub fn license_verify(certificate_str: &String) -> ReturnCode {
        unsafe {
            let code = agora_rtc_license_verify(certificate_str.as_ptr(), certificate_str.len().try_into().unwrap(), std::ptr::null(), 0);
